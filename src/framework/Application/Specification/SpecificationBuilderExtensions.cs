...
public static class SpecificationBuilderExtensions
{
        ...
        Filter? filter)
    {
        if (filter is not null)
        {
            ...
            {
                if (filter.Filters is null) throw new FshException("The Filters attribute is required when declaring a logic");
        ...
        string filterOperator)
    {
        ...
        return filterOperator switch
        {
            ...
            _ => throw new FshException("Filter Operator is not valid."),
        };
    }
        ...
        Type propertyType)
    {
        ...
        if (propertyType.IsEnum)
        {
            ...
            if (!Enum.TryParse(propertyType, stringEnum, true, out object? valueparsed)) throw new FshException(string.Format("Value {0} is not valid for {1}", value, field));
        ...
        if (propertyType == typeof(DefaultIdType))
        {
            ...
            if (!DefaultIdType.TryParse(stringGuid, out var valueparsed)) throw new FshException(string.Format("Value {0} is not valid for {1}", value, field));
    ...
    {
        if (string.IsNullOrEmpty(filter.Field)) throw new FshException("The field attribute is required when declaring a filter");
        if (string.IsNullOrEmpty(filter.Operator)) throw new FshException("The Operator attribute is required when declaring a filter");
        return filter;
    }
